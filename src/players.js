...
function Stats({ player }) {
    const {vpl} = player
    return <div style={{
        height: '100px', backgroundColor: '#000000',
        width: '100%',
        display: 'flex',
        flexDirection: 'row',
        fontWeight: 'bold',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: '1', gap: '4px', marginLeft: '8px' }}>
            <span>Matches : {vpl.matches}</span>
            <span>Runs : {vpl.runs}</span>
            <span>Average : {vpl.average}</span>
            <span>Strike Rate : {vpl.strikeRate}</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', flex: '1', gap: '4px', textAlign: 'end', marginRight: '8px' }}>
...
function Player({ player }) {
  return (
    ...
    >
      <img
        alt="example"
        src={player.img || "https://ca.slack-edge.com/T02J5FE6T-U031N7T5Q91-g79948915796-512"}
